...
scorenew <- array(0,dim=c(100,1600))
l <- 1
d <- 1
nira <- 0
for (a in 101:140)
...
for (i in 1:40)
{
  for (j in 1:40)
  {
    if (scorenew[i,j]!=0)
    {
      imposter <- c(imposter,scorenew[i,j])
    }
  }
}
cntimp <- length(imposter)
genuine <- c()
for (i in 1:40)
{
  for (j in 1:8)
  {
    if (score[i,j]!=0)
    {
      genuine <- c(imposter,scorenew[i,j])
    }
  }
}
totalmean <- c(genuine,imposter)
threshold <- mean(totalmean)
